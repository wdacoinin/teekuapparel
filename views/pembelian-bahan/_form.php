...
<div class="pembelian-bahan-t-form">

    <?php $form = ActiveForm::begin(['id' => 'pembelian-bahan-insert', 'enableAjaxValidation' => true]); ?>

    <?= $form->field($model, 'pembelian')->textInput(['type' => 'hidden', 'value' => $_GET['pembelian']])->label(false) ?>
    ...
    <?= $form->field($model, 'pembelian_jml')->textInput() ?>
    ...
    <?= $form->field($model, 'jml_now')->textInput(['type' => 'hidden'])->label(false) ?>
    ...
    $form->field($model, 'item_bonus')->widget(Select2::classname(), [
        'data' => ['0' => 'Tidak', '1' => 'Ya'],
        'options' => ['placeholder' => 'Set sebagai item bonus?', 'value' => $model->item_bonus],
        'pluginOptions' => [
            'allowClear' => true
        ],
    ]); 
