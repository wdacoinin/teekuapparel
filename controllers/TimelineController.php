...
class TimelineController extends \yii\web\Controller
{
    public function actionIndex()
    {
        ...
        $divisiAtas = $this->actionDivisiAtas();
        $divisiBawah = $this->actionDivisiBawah();

        return $this->render('index', [
            'searchModel' => $searchModel,
            'dataProvider' => $dataProvider,
            'divisiAtas' => $divisiAtas,
            'divisiBawah' => $divisiBawah,
        ]);
    }

    private function actionDivisiAtas()
    {
        $data = DivisiT::find()->where("divisi in ('8','9','10','11','17','15','16')")->asArray()->all();
        $valNew=[];
    ...
    private function actionDivisiBawah()
    {
        $data = DivisiT::find()->where("divisi in ('12','14','16')")->asArray()->all();
        $valNew=[];
        foreach($data as $key =>$value){
            $valNew[]=$value['nama'];
        }
    ...
    public function actionFlatlist()
    {
        ...
        $divisi = $_POST['divisi'];
        $this->layout = 'headerMobile';
        $data = Timelinev::find()->where(['user' => $id, 'divisi' => $divisi])->asArray()->all();
    ...
    /** Code below is copy from another Controller */
    ...
    {
        $this->layout = 'kosong';
        $model = PenjualanProdukStepT::find()->where(['penjualan_produk_step' => $_POST['penjualan_produk_step']]);
    ...
    public function actionCreate()
    {
        ...
        if ($model->load(Yii::$app->request->post())) {
            ...
            return $this->redirect(Url::to(['timeline/index']));
        } else {
    ...
    public function actionViewmobile()
    {
        ...
        if (isset($_GET['id']) &&  $_GET['id'] !== '' && $_GET['divisi'] !== '' && $_GET['penjualan_produk'] !== '') {
            ...
            $model = null;
            Yii::$app->session->setFlash('danger', 'Not Login!');
            return $this->render('viewmobile', [
                'model' => $model,
            ]);
        }
