 ...
 */
class Orderagen extends OrderT
{
    ...
    {
        return [
            [['penjualan', 'user', 'sales', 'penjualan_diskon', 'penjualan_ongkir', 'fee'], 'integer'],
            [['faktur', 'penjualan_tgl', 'penjualan_tempo', 'konsumen_nama', 'divisi', 'nama_sales', 'penjualan_status'], 'safe'],
     ...
     */
    public function search($params, $agen)
    {
        
        //if(Yii::$app->user->identity->divisi == 1 || Yii::$app->user->identity->divisi == 2 || Yii::$app->user->identity->divisi == 3){
            $query = OrderT::find()->where(['sales' => $agen])->andWhere('acc_desain = 0 AND penjualan_status != "Batal Order"')->orderBy(['penjualan' => SORT_DESC]);
        /* }else{
        ...
        $query
            //->andFilterWhere(['like', 'penjualan', $this->penjualan])
