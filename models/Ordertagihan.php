 ...
 */
class Ordertagihan extends OrderT
{
    ...
    {
        return [
            [['penjualan', 'user', 'sales', 'penjualan_diskon', 'penjualan_ongkir', 'fee', 'hprint', 'acc_desain'], 'integer'],
            [['faktur', 'penjualan_tgl', 'penjualan_tempo', 'konsumen_nama', 'divisi', 'nama_sales', 'acc_date', 'penjualan_status'], 'safe'],
    ...
    public function search($params)
    {
        ...
        if(Yii::$app->user->identity->divisi == 1 || Yii::$app->user->identity->divisi == 2 || Yii::$app->user->identity->divisi == 3){
            $query = OrderT::find()->where('acc_desain > 0 AND penjualan_status != "Batal Order"')->andWhere('penjualan_status != "Lunas"')->orderBy(['acc_date' => SORT_DESC]);
        }else{
            $query = OrderT::find()->where('acc_desain > 0 AND penjualan_status != "Batal Order"')->andWhere('penjualan_status != "Lunas"')->andWhere([ 'sales' => Yii::$app->user->identity->id ])->orderBy(['acc_date' => SORT_DESC]);
        }
        ...
        $query->andFilterWhere([
            ...
            //'fee' => $this->fee,
            'penjualan_status' => $this->penjualan_status,
            //'Total' => $this->Total,
